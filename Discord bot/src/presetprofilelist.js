...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('presetprofilelist')
        //.addStringOption(option => option.setName('id').setDescription('Name').setRequired(true))
		.setDescription('get the list of the preset profiles in the db'),
	...
	.setFooter({ text: 'Brought to you by the Izanami team', iconURL: 'https://i.pinimg.com/originals/73/c0/a8/73c0a8920f7bf77d0d9bf14c1aa73298.gif' });
		...
		var con = mysql.createConnection({
		  ...
		  var sql = `SELECT * FROM presetprofile`
            con.query(sql, function (err, result) {
            if (err) throw err;
			result.forEach(element => {
				console.log(element)
				exampleEmbed.addField(element.profile, 'empty description')
			});
